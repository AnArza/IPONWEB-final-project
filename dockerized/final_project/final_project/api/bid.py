...
from final_project.utils.parabola import optimize as parabola1
from final_project.utils.parabola2 import optimize as parabola2
...
class BidView(View):
    """
    View for creating, reading bid objects.
    """
    def get(self, request):
        ...
        for bid in bids:
            data.append({'id': bid.id,
                         ...
                         'price': str(bid.price),
                         'creative_id': bid.creative.external_id if bid.creative else '',
                         'win': bid.win,
                         'revenue': bid.revenue,
                         'current_round': bid.current_round,
                         'click_happened': bid.click_happened,
                         'conversion_happened': bid.conversion_happened,
                         'auth_ssp': bid.auth_ssp
                         })
    ...
    def post(self, request):
        ...
        """
        API endpoint that creates a new bid object.

        Args:
            request: HTTP request object containing data for the new bid object.

        Returns:
            A JSON response indicating the status of the request and the created bid object if successful.
        """
        ...
        try:
            ...
            try:
                ...
                if Config.get_solo().game_goal == "revenue":
                    op1 = parabola1(current_total_budget, bid.click_prob, bid.external_id)
                    op2 = parabola2(current_total_budget, bid.click_prob, bid.external_id)
        ...
        response = {"external_id": bid.creative.external_id, "price": float(bid.price),
                    "image_url": f"http://{ip_address}{img_parameters[2]}?width={img_parameters[0]}&height={img_parameters[1]}",
                    "cat": cats}
        bid.save()
